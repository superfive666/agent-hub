...
| # | 交付 | 完成的判据 | 状态 |
|---|---|---|---|
| M0 | 立项、ADR、`/download` 端点、控制台下载入口 | 未登录状态下能从控制台点到 `/download`；没包时按钮是禁用加说明 | ✅ 后端 + web 单测全绿 |
| M1 | 工程骨架 + 设计底盘 + `core/` 及其测试 | 舞台/玻璃板/内板/棱镜边在真机上立得住；`core` 测试全绿 | ✅ 代码就位，`core` 48 个用例全绿；**真机观感待验** |
| M2 | 首屏（填 hub 地址）+ 登录（口令 + OIDC） | 两种模式都能拿到会话并保持 | ✅ 代码就位，待真机验 |
| M3 | 对话页 | §1.1 四重信号、§1.2 层级、确认闸门都在 | ✅ 代码就位，待真机验 |
| M4 | 看板 / 待办 / 名录 | 与 web 同数据同口径 | ✅ 代码就位，待真机验 |
| M5 | 新建 todo / 新建 agent / agent 管理 / 设置（含 outbox 告警带） | §1.4 不可折叠 | ✅ 代码就位，待真机验 |
| M6 | 签名、CI 出包、发布流程跑通 | 从 tag 到 `/download` 能下到的包，全自动 | 🟡 workflow 已就位，**首次 CI 运行是第一次真正编译 app 模块** |

> ⚠️ **`app` 模块至今没有被编译过。** 立项时的开发环境访问不到 `dl.google.com`
> （Android SDK 与 AGP 插件都在那里），所以 `:app` 连配置阶段都进不去。
>
> 应对是把有需求可依的规则全部下沉到 `core/` —— 那是个不依赖 Android 的独立
> 构建，48 个用例在任何机器上都跑得了，并且已经全绿。`app/` 层做过跨文件
> 引用与泛型的静态核对，但**编译错误一定还有**。
>
> M6 的第一次 CI 运行就是第一次真正的编译，它的产出是一份错误清单，
> 修完才谈得上真机验收。**不要把上面那些「代码就位」读成「已经跑过」。**
