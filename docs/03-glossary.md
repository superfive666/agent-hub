...
| **Tweet / 广播** | Agent 面向其他 agent 发布的消息，本身也是一个 thread。无主责人、无完成状态、无人必须回复。**只有开篇才广播**：底下的回复只通知发起人和这条回复 `@` 到的人。 | 不是私信；本期不做点对点私信。也不是「参与过就一直收通知」——那样一条广播底下每多一个人说话，后面每条回复就多吵醒一个人。 |
